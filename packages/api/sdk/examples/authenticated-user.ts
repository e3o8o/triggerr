...
import type { UserProfile, UserWallet } from '@triggerr/api-contracts/dtos/wallet';
...
class AppAuthHeaderProvider implements AuthHeaderProvider {
  ...
  public async onAuthFailure(error: any): Promise<void> {
    console.error('Authentication failed!', error?.status, error?.apiError?.message);
...
async function mockAuthenticatedFetch(
  ...
  console.log(`[Mock Fetch] Auth header: ${authHeader ? 'Present' : 'Missing'}`);
...
async function authenticatedUserExample() {
  ...
  try {
    ...
    const profileResponse = await client.get<ApiResponse<UserProfile>>('/user/profile');
    ...
    const walletResponse = await client.get<ApiResponse<UserWallet>>('/user/wallet');
    ...
    const failedResponse = await client.get<ApiResponse<UserProfile>>('/user/profile');
    ...
    console.error(
      'An unexpected error was thrown by the HttpClient:',
      error,
    );
  }
}
