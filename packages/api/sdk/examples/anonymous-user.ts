...
class AnonymousAuthProvider implements AuthHeaderProvider {
  ...
  public async getAuthHeaders(): Promise<Record<string, string> | null> {
    ...
    if (sessionId) {
      console.log(`[Auth Provider] Supplying anonymous session ID: ${sessionId}`);
      return {
  ...
  public async onAuthFailure(error: any): Promise<void> {
    ...
    console.warn('[Auth Provider] Anonymous session was considered invalid.', error);
...
async function anonymousUserExample() {
  ...
  try {
    ...
    console.log(`Current anonymous session ID: ${sessionManager.getSessionId()}`);
    ...
    const quoteResponse = await client.post<ApiResponse<InsuranceQuoteResponse>, InsuranceQuoteRequest>(
      '/insurance/quote',
      quoteRequest,
    );
    ...
    if (quoteResponse.success) {
      ...
      const purchaseResponse = await client.post<ApiResponse<PolicyPurchaseResponseDto>, PolicyPurchaseRequestDto>(
        '/insurance/purchase',
        purchaseRequest,
      );
      ...
      if (purchaseResponse.success) {
        ...
        console.log('Policy purchased successfully! Tracking Number:', trackingNumber);
        ...
        if (policyStatusResponse.success) {
          console.log('Policy status retrieved successfully:', policyStatusResponse.data?.data);
        } else {
...
async function mockAnonymousFetch(
  ...
  if (url.endsWith('/insurance/purchase')) {
    return new Response(JSON.stringify({
      success: true,
      data: {
        data: { policy_id: `pol_anon_${Date.now()}`, tracking_number: `track_anon_${Date.now()}`, status: 'ACTIVE' }
      }
    }), { status: 200, headers: { 'Content-Type': 'application/json' } });
  }
  ...
  if (url.includes('/policies/track/')) {
    ...
    return new Response(JSON.stringify({
      success: true,
      data: {
        data: { id: `pol_for_${trackingNumber}`, tracking_number: trackingNumber, status: 'ACTIVE', premium: 25.50 }
      }
    }), { status: 200, headers: { 'Content-Type': 'application/json' } });
  }

  return new Response(JSON.stringify({ success: false, error: { code: 'NOT_FOUND', message: 'Endpoint not mocked' } }), { status: 404 });
}
