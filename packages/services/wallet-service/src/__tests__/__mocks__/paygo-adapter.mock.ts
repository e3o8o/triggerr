...
const mockPayGoClientInstance = {
  ...
  signAndPostTransactionFromParams: mock(() =>
    Promise.resolve({
      success: true,
      hash: "0xmock_transaction_hash" as Hex,
    }),
  ),
  setPk: mock(() => {}),
};
...
// Mock static methods
const mockGenerateNewWallet = mock(() =>
  Promise.resolve({
    privateKey:
...
const mockCreateWithNewWallet = mock(() =>
  Promise.resolve({
...
const MockPayGoClientService = Object.assign(
  mock().mockImplementation(() => mockPayGoClientInstance),
  {
...
const mockConvertToPayGoAmount = mock((amount: string) => {
...
const MockTransfer = mock().mockImplementation(
  (recipient: Hex, amount: bigint) => ({
    recipient,
    amount,
    type: "Transfer",
  }),
...
const MockCreateEscrow = mock().mockImplementation((params: any) => ({
  ...params,
  type: "CreateEscrow",
}));

const MockFulfillEscrow = mock().mockImplementation((params: any) => ({
  ...params,
  type: "FulfillEscrow",
}));

const MockReleaseEscrow = mock().mockImplementation((params: any) => ({
  ...params,
  type: "ReleaseEscrow",
}));

const MockFaucetRequest = mock().mockImplementation((amount: bigint) => ({
  amount,
  type: "FaucetRequest",
}));
...
const mockInitPayGoClient = mock((privateKey: Hex) =>
  Promise.resolve(mockPayGoClientInstance),
);
const mockInitPayGoClientWithNewWallet = mock(() =>
  Promise.resolve({
    client: mockPayGoClientInstance,
    privateKey:
      "0xabcdef1234567890abcdef1234567890abcdef1234567890abcdef1234567890" as Hex,
