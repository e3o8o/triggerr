import { pgTable, text, timestamp, boolean, integer, decimal, jsonb, varchar, char, pgEnum, index, unique, serial, inet, check, } from "drizzle-orm/pg-core";
...
export const flight = pgTable("flight", {
    ...
    airlineIcaoCode: char("airline_icao_code", { length: 3 }).references(() => airline.icaoCode), // Prefer ICAO for airline ref if available & unique
    ...
    aircraftIcaoCode: varchar("aircraft_icao_code", { length: 4 }).references(() => aircraftTypes.icaoCode),
    ...
    unique("unique_flight_schedule").on(table.flightNumber, table.departureScheduledAt, table.departureAirportIataCode, table.airlineIcaoCode),
]);
// ============================================================================
...
export const policy = pgTable("policy", {
    ...
    check("policy_user_check", sql `(user_id IS NOT NULL) != (anonymous_session_id IS NOT NULL)`),
]);
export const policyVerificationCode = pgTable("policy_verification_code", {
    id: text("id").primaryKey(),
...
export const revenueSharingRule = pgTable("revenue_sharing_rule", {
    id: text("id")
        .primaryKey()
        .default(sql `generate_ulid()`),
    ...
    providerProductId: text("provider_product_id").references(() => providerProduct.id, { onDelete: "cascade" }),
    ...
    unique("rev_share_provider_prod_from_unique").on(table.providerId, table.providerProductId, table.applicableFrom),
...
export const webhook = pgTable("webhook", {
    ...
    subscribedEvents: jsonb("subscribed_events")
        .$type() // Ideally, map to webhookEventTypeEnum values
        .notNull(),
...
export const escrowPool = pgTable("escrow_pool", {
    ...
    // Pool Status & Health
    isActive: boolean("is_active").notNull().default(true),
    healthScore: decimal("health_score", { precision: 5, scale: 4 }).default("1.0000"), // 0-1 health rating
...
export const escrowPoolParticipant = pgTable("escrow_pool_participant", {
    ...
    preferredCategories: jsonb("preferred_categories").$type(), // Flight types they want to cover
    // Returns & Performance
...
// ============================================================================
export const flightDataSource = pgTable("flight_data_source", {
    id: text("id").primaryKey(),
...
export const routes = pgTable("routes", {
    ...
    // <--- CHANGED to return an ARRAY
    unique("unique_operational_route").on(table.airlineIcaoCode, table.sourceAirportIataCode, table.destinationAirportIataCode),
...
export const historicalFlightSegments = pgTable("historical_flight_segments", {
    ...
    "scheduled_departure_timestamp_utc", { withTimezone: true }).notNull(),
    estimatedDepartureTimestampUTC: timestamp(
    // FlightAware: estimated_out
    "estimated_departure_timestamp_utc", { withTimezone: true }),
    actualDepartureTimestampUTC: timestamp("actual_departure_timestamp_utc", {
        // FlightAware: actual_out (gate departure)
        withTimezone: true,
    }),
    gateDepartureScheduledTimestampUTC: timestamp("gate_departure_scheduled_utc", { withTimezone: true }), // FlightAware: scheduled_out
    gateDepartureActualTimestampUTC: timestamp("gate_departure_actual_utc", {
        withTimezone: true,
    }), // FlightAware: actual_out
    runwayDepartureScheduledTimestampUTC: timestamp("runway_departure_scheduled_utc", { withTimezone: true }), // FlightAware: scheduled_off
    runwayDepartureActualTimestampUTC: timestamp("runway_departure_actual_utc", { withTimezone: true }), // FlightAware: actual_off
    ...
    runwayArrivalScheduledTimestampUTC: timestamp("runway_arrival_scheduled_utc", { withTimezone: true }), // FlightAware: scheduled_on
...
export const historicalWeatherObservations = pgTable("historical_weather_observations", {
    ...
    precipitationProbabilityPercent: integer("precipitation_probability_percent"), // For forecast
    ...
    // <--- CHANGED to return an ARRAY
    index("hwo_airport_time_idx").on(table.airportIataCode, table.observationTimestampUTC, table.forecastPeriod),
...
export const rawApiCallLogs = pgTable("raw_api_call_logs", {
    ...
    associatedFlightId: text("associated_flight_id").references(() => flight.id), // Optional link to an operational flight
    associatedPolicyId: text("associated_policy_id").references(() => policy.id), // Optional link
}, (table) => [
    // <--- CHANGED to return an ARRAY
...
export const userWallets = pgTable("user_wallets", {
    ...
    check("user_wallets_user_check", sql `(user_id IS NOT NULL) != (anonymous_session_id IS NOT NULL)`),
...
export const userPaymentMethods = pgTable("user_payment_methods", {
    ...
    check("user_payment_methods_user_check", sql `(user_id IS NOT NULL) != (anonymous_session_id IS NOT NULL)`),
]);
...
export const conversationMessages = pgTable("conversation_messages", {
    ...
    userId: text("user_id").references(() => user.id, { onDelete: "set null" }),
    anonymousSessionId: text("anonymous_session_id").references(() => session.id),
...
export const providerProductRelations = relations(providerProduct, ({ one, many }) => ({
    provider: one(provider, {
...
export const policyVerificationCodeRelations = relations(policyVerificationCode, ({ one }) => ({
    policy: one(policy, {
...
export const revenueSharingRuleRelations = relations(revenueSharingRule, ({ one }) => ({
    provider: one(provider, {
...
export const escrowPoolParticipantRelations = relations(escrowPoolParticipant, ({ one }) => ({
    pool: one(escrowPool, {
...
export const historicalFlightSegmentsRelations = relations(historicalFlightSegments, ({ one }) => ({
    airline: one(airline, {
...
export const historicalWeatherObservationsRelations = relations(historicalWeatherObservations, ({ one }) => ({
    airport: one(airport, {
...
export const systemConfigurationRelations = relations(systemConfiguration, () => ({
