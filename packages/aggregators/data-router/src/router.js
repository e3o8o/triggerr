...
export class DataRouter {
    flightAggregator;
    weatherAggregator;
    maxConcurrentWeatherRequests;
    defaultIncludeWeather;
    timeoutMs;
    constructor(flightAggregator, weatherAggregator, config = {}) {
        ...
        console.log(`[DataRouter] Initialized with flight and weather aggregators, max concurrent weather requests: ${this.maxConcurrentWeatherRequests}`);
    }
    /**
     * Main method to get all data required for policy generation.
     * This orchestrates both flight and weather data collection.
     */
    async getDataForPolicy(request) {
        const startTime = Date.now();
        const { flightNumber, date, airports, includeWeather = this.defaultIncludeWeather, weatherCoordinates, } = request;
        console.log(`[DataRouter] Starting data collection for policy: flight ${flightNumber} on ${date}`);
        try {
            // Step 1: Get flight data (this is always required)
            const flightResult = await this.getFlightData(flightNumber, date);
            console.log(`[DataRouter] Flight data collected from ${flightResult.sourcesUsed.join(", ")} (quality: ${flightResult.qualityScore.toFixed(3)})`);
            // Step 2: Get weather data if required
            let weatherResults = [];
            if (includeWeather) {
                weatherResults = await this.getWeatherData(flightResult.data, airports, weatherCoordinates, date);
                console.log(`[DataRouter] Weather data collected for ${weatherResults.length} locations`);
            }
            const totalProcessingTime = Date.now() - startTime;
            console.log(`[DataRouter] Successfully collected all data for policy (${totalProcessingTime}ms)`);
            ...
            const totalProcessingTime = Date.now() - startTime;
            console.error(`[DataRouter] Failed to collect policy data for flight ${flightNumber}:`, error);
            throw new Error(`Policy data collection failed for ${flightNumber}: ${error instanceof Error ? error.message : "Unknown error"}`);
        }
    }
    /**
    ...
    async getFlightData(flightNumber, date) {
        try {
            ...
            if (result.qualityScore < 0.3) {
                console.warn(`[DataRouter] Flight data quality is low (${result.qualityScore.toFixed(3)}), but proceeding`);
            }
            return result;
        }
        catch (error) {
            console.error(`[DataRouter] Flight data collection failed:`, error);
            throw new Error(`Failed to collect flight data: ${error instanceof Error ? error.message : "Unknown error"}`);
        }
    }
    /**
     * Get weather data for relevant locations using the WeatherAggregator.
     */
    async getWeatherData(flightData, providedAirports, providedCoordinates, date) {
        try {
            // Determine locations to fetch weather for
            const weatherLocations = this.determineWeatherLocations(flightData, providedAirports, providedCoordinates);
            if (weatherLocations.length === 0) {
                console.warn(`[DataRouter] No weather locations determined for flight ${flightData.flightNumber}`);
                return [];
            }
            console.log(`[DataRouter] Collecting weather data for ${weatherLocations.length} locations: ${weatherLocations.map((l) => l.name || "coordinates").join(", ")}`);
            // Create weather requests with concurrency limit
            const weatherPromises = this.createWeatherRequests(weatherLocations, date);
            // Execute weather requests with concurrency control
            const weatherResults = await this.executeConcurrentWeatherRequests(weatherPromises);
            // Filter out failed requests
            const successfulResults = weatherResults.filter((result) => result !== null);
            if (successfulResults.length === 0) {
                console.warn(`[DataRouter] No successful weather data collection for any location`);
            }
            ...
            console.warn(`[DataRouter] Proceeding without weather data due to error: ${error instanceof Error ? error.message : "Unknown error"}`);
            return [];
        }
    }
    /**
     ...
     */
    determineWeatherLocations(flightData, providedAirports, providedCoordinates) {
        const locations = [];
        ...
        if (providedAirports && providedAirports.length > 0) {
            providedAirports.forEach((airport) => {
                ...
                if (coords) {
                    ...
                    console.warn(`[DataRouter] No coordinates found for airport ${airport}`);
                }
            });
            return locations;
        }
     ...
     */
    createWeatherRequests(locations, date) {
        return locations.map(async (location) => {
            try {
                const weatherIdentifier = {
                ...
                console.warn(`[DataRouter] Weather request failed for ${location.name}:`, error);
                return null;
            }
        });
    }
     ...
     * Execute weather requests with concurrency control.
     */
    async executeConcurrentWeatherRequests(promises) {
        const results = [];
        // Process promises in batches to respect concurrency limit
        for (let i = 0; i < promises.length; i += this.maxConcurrentWeatherRequests) {
            const batch = promises.slice(i, i + this.maxConcurrentWeatherRequests);
            console.log(`[DataRouter] Processing weather batch ${Math.floor(i / this.maxConcurrentWeatherRequests) + 1} with ${batch.length} requests`);
            ...
            for (const result of batchResults) {
                if (result.status === "fulfilled") {
                    ...
                    console.warn(`[DataRouter] Weather request in batch failed:`, result.reason);
                    results.push(null);
                }
            }
        }
     ...
     */
    getLocationName(weatherData, index) {
        if (weatherData.airportIataCode &&
     ...
     */
    createTimeoutPromise(operationName) {
     ...
     */
    static addAirportCoordinates(airportCode, coordinates) {
        AIRPORT_COORDINATES[airportCode.toUpperCase()] = coordinates;
        console.log(`[DataRouter] Added coordinates for airport ${airportCode.toUpperCase()}`);
    }
    /**
     * Get available airport coordinates.
     */
    static getAvailableAirports() {
