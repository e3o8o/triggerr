...
} from '@triggerr/api-contracts/validators/insurance';
import type {
  PaymentConfirmation
} from '@triggerr/api-contracts/dtos/insurance';
...
export async function createOrGetInsuranceProduct(
  productName?: string,
  description?: string
): Promise<{ success: true; product: StripeProduct } | { success: false; error: string }> {
  const defaultName = productName || stripeSettings.products.defaultProductData.name;
  const defaultDescription = description || stripeSettings.products.defaultProductData.description;
...
export async function createStripePrice(
  ...
  amountInCents: number,
  currency: string = 'usd'
): Promise<{ success: true; price: StripePrice } | { success: false; error: string }> {
...
export async function createPolicyCheckoutSession(
...
): Promise<{ success: true; data: StripePurchaseInitiationResponse } | { success: false; error: string }> {
  ...
  if (params.premiumInCents < 50) { // Stripe minimum is $0.50
  ...
  if (!productResult.success) {
    return { success: false, error: `Failed to create product: ${productResult.error}` };
  }
  ...
  if (!priceResult.success) {
    return { success: false, error: `Failed to create price: ${priceResult.error}` };
  }
  ...
  const metadata: Record<string, string> = {
    ...
    ...(params.flightDetails && {
      flightNumber: params.flightDetails.flightNumber.substring(0, stripeSettings.metadata.maxValueLength),
      airline: params.flightDetails.airline.substring(0, stripeSettings.metadata.maxValueLength),
  ...
  if (!session.url) {
    return { success: false, error: 'Stripe checkout session URL not generated' };
  }
