...
export class GoogleWeatherClient {
    ...
    baseUrl = "https://weather.googleapis.com/v1/forecast/days:lookup";
    apiKey;
     ...
     */
    async fetchWeather(coordinates, date) {
     ...
     */
    async fetchWeatherByAirport(airportCode, date) {
     ...
     */
    transformToCanonical(forecast, coordinates) {
        const now = new Date().toISOString();
        // Get observation timestamp for today
        const observationDate = new Date(forecast.displayDate.year, forecast.displayDate.month - 1, // Month is 1-based in Google API
        ...
        const tempCelsius = this.convertToCelsius(forecast.maxTemperature.degrees, forecast.maxTemperature.unit);
     ...
     */
    mapWeatherCondition(conditionType) {
        if (!conditionType)
            return "UNKNOWN";
     ...
     */
    determinePrecipitationType(condition) {
        switch (condition) {
            case "LIGHT_RAIN":
            case "MODERATE_RAIN":
            case "HEAVY_RAIN":
     ...
     */
    calculateQualityScore(forecast, primaryForecast) {
        let score = 0;
        let maxScore = 0;
        ...
        if (forecast.feelsLikeMaxTemperature)
            score += 0.5;
        if (forecast.moonEvents)
            score += 0.5;
