...
export const FlightSearchInputSchema = z.object({
  ...
  data.flightNumber || (data.origin && data.destination && data.date) || data.naturalLanguageQuery,
  { message: "Must provide flight number, or origin/destination/date, or natural language query" }
);
...
export const InsuranceQuoteRequestSchema = z.object({
  ...
  insurancePreferences: z.object({
    ...
    delayThresholdMinutes: z.enum(COVERAGE.AVAILABLE_DELAY_THRESHOLDS.map(String) as [string, ...string[]]).optional(),
...
export const AddToCartRequestSchema = z.object({
  ...
  flightContextSnapshot: z.object({ // Snapshot of the flight details at time of quote
...
// ===========================================================================
// INFER TYPES FROM SCHEMAS (for use in application code)
// ===========================================================================
...
// Type exports removed to avoid conflicts with api-types.ts
// Use the types from @triggerr/shared/types instead
// export type StripeWebhookPayload = z.infer<typeof StripeWebhookPayloadSchema>;
// export type PayGoWebhookPayload = z.infer<typeof PayGoWebhookPayloadSchema>;
